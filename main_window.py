...
QPushButton { background-color: #4a4a6a; border: none; border-radius: 8px; padding: 8px 16px; color: #e0e0e0; font-weight: bold; }
...
QComboBox { background-color: #2a2a4a; border: 1px solid #4a4a6a; border-radius: 8px; padding: 6px 12px; color: #e0e0e0; }
...
QTabBar::tab { background-color: #2a2a4a; color: #aaaacc; padding: 8px 16px; border-top-left-radius: 6px; border-top-right-radius: 6px; margin-right: 2px; }
...
QHeaderView::section { background-color: #2a2a4a; color: #e0e0e0; padding: 8px; border: 1px solid #4a4a6a; }
                ...
                for item in raw_results:
                    if isinstance(item, dict):
                        desc = (item.get('description') or item.get('message') or
                                f"{item.get('vulnerability', '')} {item.get('evidence', '')} {item.get('recommendation', '')}".strip())
                        if not desc:
                            desc = str(item)
                        formatted.append({
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        top_layout.addWidget(self.scan_type_combo)
    ...
    def start_scan(self):
        ...
        try:
            ...
            if 'Basic' in scan_type_text:
                scan_type = 'basic'
            elif 'Premium' in scan_type_text:
                scan_type = 'premium'
            else:
